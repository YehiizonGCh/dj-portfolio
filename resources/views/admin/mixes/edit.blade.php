...
@section('content')
    ...
    <div class="bg-white/5 rounded-xl border border-white/5 p-6 max-w-2xl">
        ...
        <form method="POST" action="{{ route('admin.mixes.update', $mix) }}" enctype="multipart/form-data" class="space-y-5">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
                ...
                <div class="sm:col-span-2">
                    ...
                    <input type="text" name="title" value="{{ old('title', $mix->title) }}"
                ...
                <div>
                    ...
                    <select name="platform" class="w-full bg-[#1a1a1a] border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-cyan-400">
                        <option value="soundcloud" {{ old('platform', $mix->platform) === 'soundcloud' ? 'selected' : '' }}>SoundCloud</option>
                        <option value="mixcloud" {{ old('platform', $mix->platform) === 'mixcloud' ? 'selected' : '' }}>Mixcloud</option>
                        <option value="youtube" {{ old('platform', $mix->platform) === 'youtube' ? 'selected' : '' }}>YouTube</option>
                    </select>
                </div>

                <div>
                    <label class="block text-xs font-semibold text-gray-500 uppercase tracking-widest mb-1.5">Género</label>
                    <input type="text" name="genre" value="{{ old('genre', $mix->genre) }}" placeholder="Tech House, Deep Techno..."
                ...
                <div class="sm:col-span-2">
                    ...
                    <input type="text" name="embed_url" value="{{ old('embed_url', $mix->embed_url) }}"
                           class="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-cyan-400">
                ...
                <div>
                    ...
                    <input type="date" name="recorded_at" value="{{ old('recorded_at', $mix->recorded_at?->format('Y-m-d')) }}"
                           class="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-cyan-400">
                </div>
                ...
                <div class="sm:col-span-2">
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" name="is_featured" value="1"
                               {{ old('is_featured', $mix->is_featured) ? 'checked' : '' }}
